...
export const TricksEntryBox: React.FC<TricksEntryBoxProps> = (props) => {
  const { usTricks, themTricks, setUsTricks, setThemTricks, onSubmit, submitting } = props;
  return (
    <form className="flex flex-row items-center gap-4 w-full max-w-xl justify-center" onSubmit={e => { e.preventDefault(); onSubmit(); }}>
      <div className="flex flex-row gap-4 w-full max-w-xs justify-between">
        <label className="flex flex-col items-center w-1/2">
          ...
          <input
            ...
            min={0}
            value={usTricks}
            onChange={e => setUsTricks(Number(e.target.value))}
        ...
        <label className="flex flex-col items-center w-1/2">
          ...
          <input
            ...
            min={0}
            value={themTricks}
            onChange={e => setThemTricks(Number(e.target.value))}
            className="border rounded px-2 py-1 w-20 text-center text-lg"
            required
          />
        </label>
      ...
      <button
        ...
        disabled={submitting}
      >
        {submitting ? 'Submitting...' : 'Submit Tricks'}
      </button>
    </form>
  );
};

...
const BidEntryBox: React.FC<BidEntryBoxProps> = (props) => {
  const { selected, onSelect, bid, setBid, onSubmit, submitting } = props;
  return (
    <form
      ...
      <div className="flex flex-row gap-2">
        {SEATS.map(seat => (
          <button
            key={seat}
            type="button"
            className={`px-4 py-2 rounded-lg font-bold border-2 focus:outline-none transition-all duration-150 ${selected === seat ? "bg-blue-500 text-white border-blue-700 shadow" : "bg-white text-gray-700 border-gray-300 hover:bg-blue-100"}`}
